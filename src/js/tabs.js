const tabHandlers = () => {
		...
		tabbeds.forEach((tabbed) => {
			...
			tabs.forEach((tab) => {
				...
				tab.addEventListener("blur", (e) => {
					...
					if (
						!e.relatedTarget.parentNode.classList.contains("tabbed") &&
						!e.relatedTarget.closest(".tabbed")
					) {
						closeTabs(tabs, panels);
					}
				});
	...
	function tabTabTrap(tab, nextTab, previousTab, panel) {
		const tabIndexes = panel.querySelectorAll(
		...
		[tab, panel].forEach((el) => {
			el.addEventListener("keydown", (e) => {
				if (e.key === "Tab") {
					if (e.shiftKey) {
						/* shift + tab */ if (document.activeElement === tab) {
							if (previousTab) {
								previousTab.focus();
								e.preventDefault();
							}
